...
if (AFFILATE_INDIVIDUAL_PERCENTAGE == 'true') {
    $affiliate_commission_query = xtc_db_query ("select affiliate_commission_percent from " . TABLE_AFFILIATE . " where affiliate_id = '" . $_SESSION['affiliate_ref'] . "'");
    $affiliate_commission = xtc_db_fetch_array($affiliate_commission_query);
...
if (isset($_SESSION['affiliate_ref'])) {
	$sql_data_array = array('affiliate_id' => $_SESSION['affiliate_ref'],
                            ...
                            'affiliate_level' => '0');
    xtc_db_perform(TABLE_AFFILIATE_SALES, $sql_data_array);

    if (AFFILATE_USE_TIER == 'true') {
    	$affiliate_tiers_query = xtc_db_query ("SELECT aa2.affiliate_id, (aa2.affiliate_rgt - aa2.affiliate_lft) as height
