  ...
  while (list($key, ) = each($_GET)) {
    switch ($key) {
      case 'banner':
        ...
        $banner_query = xtc_db_query("select affiliate_banners_title, affiliate_banners_image, affiliate_banners_html_text from " . TABLE_AFFILIATE_BANNERS . " where affiliate_banners_id = '" . xtc_db_input($banners_id) . "'");
        ...
        if ($banner['affiliate_banners_html_text']) {
          ...
          $image_source = xtc_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . $banner['affiliate_banners_image'], $page_title);
        }
        break;
    }
  }
