...
// If an order is deleted delete the sale too (optional)
  if ($_GET['action'] == 'deleteconfirm' && basename($_SERVER['SCRIPT_FILENAME']) == FILENAME_ORDERS && AFFILIATE_DELETE_ORDERS == 'true') {
    $affiliate_oID = xtc_db_prepare_input($_GET['oID']);
    xtc_db_query("delete from " . TABLE_AFFILIATE_SALES . " where affiliate_orders_id = '" . xtc_db_input($affiliate_oID) . "' and affiliate_billing_status != 1");
  }
?>
